...
async function storeCustomerPurchase(
  ...
  try {
    ...
    logEvent({
      level: LogLevel.INFO,
      message: 'Stored customer purchase in database',
      data: { customerId }
    }, correlationId);
  } catch (error) {
    logError(
...
function isRetryableError(error: unknown, config: RetryConfig): boolean {
  ...
  const errorCode = (error as unknown as Record<string, unknown>).code as string | undefined; // Some errors have a code property
...
/**
 * Handle successful payment intents for marketplace transactions
 * Updates marketplace transaction status and grants access
 */
async function handlePaymentIntentSucceeded(paymentIntent: Stripe.PaymentIntent, correlationId: string) {
  logEvent({
    level: LogLevel.INFO,
    message: 'Processing successful payment intent for marketplace transactions',
    eventId: paymentIntent.id,
    data: { 
      paymentIntentId: paymentIntent.id,
      amount: paymentIntent.amount,
      currency: paymentIntent.currency,
      operation: 'marketplace_payment_success'
    }
  }, correlationId);

  try {
    // Update marketplace transactions by payment intent ID
    ...
    if (directError) {
      logError(
        ErrorCategory.DATABASE,
        'Failed to update marketplace transaction by payment intent ID',
        directError,
        correlationId,
        { 
          paymentIntentId: paymentIntent.id,
          operation: 'marketplace_update_failed'
        }
      );
    } else if (directUpdate && directUpdate.length > 0) {
      logEvent({
        level: LogLevel.INFO,
        message: 'Successfully updated marketplace transaction',
        eventId: paymentIntent.id,
        data: { 
          transactionId: directUpdate[0].id,
          paymentIntentId: paymentIntent.id,
          operation: 'marketplace_update_success'
        }
      }, correlationId);
    } else {
      logEvent({
        level: LogLevel.INFO,
        message: 'No marketplace transaction found for payment intent',
        eventId: paymentIntent.id,
        data: { 
          paymentIntentId: paymentIntent.id,
          operation: 'marketplace_no_transaction'
        }
      }, correlationId);
    }
    ...
    logError(
      ErrorCategory.WEBHOOK_PROCESSING,
      'Error processing payment intent success for marketplace',
      error,
      correlationId,
      { 
        paymentIntentId: paymentIntent.id,
        operation: 'marketplace_payment_processing'
      }
    );
    throw error;
  }
      ...
      async () => {
        ...
        switch (event.type) {
          case 'checkout.session.completed':
            ...
            // Also handle credits purchases
            await handleCreditsPurchase(event.data.object as Stripe.Checkout.Session, correlationId);
            break;
  ...
  await executeWithRetry(
    async () => {
      ...
      if (existingCustomers.data.length > 0) {
        ...
        // Parse existing purchase history
        ...
        const updatedPurchaseHistory = [...existingPurchaseHistory, newPurchase];
        ...
        customer = await stripe.customers.update(customer.id, {
          ...
          metadata: {
            ...
            purchase_history: JSON.stringify(updatedPurchaseHistory),
            updated_at: new Date().toISOString(),
          }
        });
        
        // Store purchase in Supabase
        await storeCustomerPurchase(customer.id, newPurchase, correlationId);
        ...
        // Store purchase in Supabase
        await storeCustomerPurchase(customer.id, purchaseHistory[0], correlationId);
      }
...
/**
 * Handle credits purchases from checkout sessions
 * Processes credits purchases and updates user balances
 */
async function handleCreditsPurchase(session: Stripe.Checkout.Session, correlationId: string) {
  logEvent({
    level: LogLevel.INFO,
    message: 'Processing credits purchase from checkout session',
    sessionId: session.id,
    data: { 
      operation: 'credits_purchase_check',
  ...
  try {
    ...
    if (md.kind !== "credits_purchase") {
      logEvent({
        level: LogLevel.INFO,
        message: 'Not a credits purchase, skipping credits handling',
        sessionId: session.id,
        data: { 
          kind: md.kind,
          operation: 'credits_skip'
        }
      }, correlationId);
      return;
    }
    ...
    logEvent({
      level: LogLevel.INFO,
      message: 'Credits purchase details extracted',
      sessionId: session.id,
      data: { 
    ...
    if (!userId || !credits || credits <= 0 || !piId) {
      logEvent({
        level: LogLevel.WARN,
        message: 'Invalid credits purchase payload, skipping',
        sessionId: session.id,
        data: { 
          userId: !!userId,
          credits,
          paymentIntentId: !!piId,
          operation: 'credits_skip_invalid'
        }
      }, correlationId);
      return;
    }

    // Try to write the ledger entry
    const { data: ledgerData, error: ledgerErr } = await supabase
      ...
      .insert({
        ...
        reason: "purchase",
        reference_id: piId,
        metadata: { amount_cents, payment_intent: piId }
      })
      .select("reference_id");

    if (ledgerErr) {
      if ((ledgerErr as any).code === "23505") {
        // Duplicate reference_id → credits already granted
        logEvent({
          level: LogLevel.INFO,
          message: 'Credits already granted for this payment intent',
          sessionId: session.id,
          data: { 
            paymentIntentId: piId,
            operation: 'credits_duplicate_skip'
          }
        }, correlationId);
        return;
      }
      ...
      logError(
        ErrorCategory.DATABASE,
        'Failed to insert credits ledger entry',
        ledgerErr,
        correlationId,
        { 
          sessionId: session.id,
    ...
    if (!ledgerData || ledgerData.length === 0) {
      logEvent({
        level: LogLevel.WARN,
        message: 'Credits ledger insert returned no rows, skipping increment',
        sessionId: session.id,
        data: { 
          operation: 'credits_skip_no_data'
        }
      }, correlationId);
      return;
    }
    ...
    logEvent({
      level: LogLevel.INFO,
      message: 'Credits ledger entry created successfully',
      sessionId: session.id,
      data: { 
        paymentIntentId: piId,
        operation: 'credits_ledger_success'
      }
    }, correlationId);
    ...
    // Handle case where profile doesn't exist (first-time user)
    if (readErr && (readErr as any).code !== "PGRST116") {
      logError(
        ErrorCategory.DATABASE,
        'Failed to read user profile for credits update',
        readErr,
        correlationId,
        { 
          sessionId: session.id,
    ...
    if (upErr) {
      logError(
        ErrorCategory.DATABASE,
        'Failed to update user profile credits',
        upErr,
        correlationId,
        { 
          sessionId: session.id,
    ...
    logEvent({
      level: LogLevel.INFO,
      message: 'Credits successfully granted to user',
      sessionId: session.id,
      data: { 
        userId,
        ...
        newBalance: next,
        paymentIntentId: piId,
        operation: 'credits_grant_success'
      }
    }, correlationId);
    ...
    logError(
      ErrorCategory.WEBHOOK_PROCESSING,
      'Error processing credits purchase',
      error,
      correlationId,
      { 
        sessionId: session.id,
    ...
    for (const item of marketplaceItems) {
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          buyer_id: session.customer_details?.email || null, // Use email as buyer identifier
      ...
      if (updateError) {
        logError(
          ErrorCategory.DATABASE,
          'Failed to update marketplace listing status',
          updateError,
          correlationId,
          { 
            sessionId: session.id,
