...
import { Pencil, Check, X, Sparkles, Trash2,XCircle, Loader2, AlertTriangle, ChevronDown, Upload, Plus, Package, CheckCircle, MessageSquare, Download } from "lucide-react"
...
async function fetchTotalSales(userId: string) {
  ...
  try {
    // 1. Get total sales count (all time) from asset_buyers
    ...
    if (totalSalesError) {
      ...
      setTotalSalesCount(totalSalesData?.length || 0)
    }

    // 2. Get asset_buyers data to calculate available revenue
    const { data: assetBuyersData, error: assetBuyersError } = await supabase
      .from('asset_buyers')
      .select('revenue_status, purchase_amount_cents')
      .eq('seller_id', userId)

    if (assetBuyersError) {
      console.error('Error fetching asset buyers:', assetBuyersError)
      setTotalSales(0)
      setTotalRevenue(0)
    } else {
      // Count available sales (status = 'available')
      const availableSalesCount = assetBuyersData?.filter(ab => ab.revenue_status === 'available').length || 0
      const totalRevenueCents = availableSalesCount * 200 // $2 per sale
      
      setTotalSales(availableSalesCount)
      setTotalRevenue(totalRevenueCents)
      
      console.log('Asset buyers data:', {
        totalSalesCount: totalSalesData?.length || 0,
        availableSales: availableSalesCount,
        totalRevenue: totalRevenueCents,
        assetBuyersData: assetBuyersData
      })
    }
...
async function fetchUserCredits(userId: string) {
  try {
    ...
    if (profileError) {
      ...
      console.log('🔍 User credits fetched:', profileData?.credits || 0)
    }
  } catch (error) {
...
async function convertRevenueToCredits() {
  try {
    ...
    const { data: salesToUpdate, error: salesError } = await supabase
      .from('asset_buyers')
      .select('id')
      .eq('seller_id', uid)
    ...
    if (salesError) {
      console.error('Error fetching sales to update:', salesError)
      throw salesError
    }

    // Initialize saleIds array - will be empty if no sales found
    const saleIds = salesToUpdate?.map(sale => sale.id) || []

    // Only proceed with tracking updates if there are sales to process
    if (saleIds.length > 0) {
      // Third, update revenue_tracking table to mark sales as 'credited' and link to revenue request
      ...
      const missingTrackingIds = saleIds.filter(id => !existingTrackingIds.includes(id))
    ...
    // Also update asset_buyers table to keep them in sync
    // Only update sales that were actually converted to credits (available status)
    console.log('🔍 Updating asset_buyers for credit conversion:', {
      saleIds: saleIds,
      revenueRequestId: revenueRequestId,
      note: 'Only updating sales with revenue_status = available'
    })
    
    const { error: assetBuyersError } = await supabase
      .from('asset_buyers')
      .update({ 
        revenue_status: 'credited',
        revenue_request_id: revenueRequestId
    ...
    // Refresh user credits and revenue data
    if (uid) {
      await fetchUserCredits(uid)
      // Refresh revenue data by refetching it
      await fetchTotalSales(uid)
    }
...
async function handleStripeFormSubmit() {
  ...
  try {
    ...
    // Get the available sales to update
    const { data: availableSales, error: salesError } = await supabase
      .from('asset_buyers')
      .select('id')
      .eq('seller_id', uid)
      .eq('revenue_status', 'available')
    ...
    if (salesError) {
      console.error('Error fetching available sales:', salesError)
      throw salesError
    }
    ...
    const saleIds = availableSales.map(sale => sale.id)
    ...
    if (updateError) {
      console.error('Error updating asset_buyers revenue status:', updateError)
      throw updateError
    }

    // Add debug logging
    console.log('🔍 Asset buyers update result:', {
      saleIds: saleIds,
      revenueRequestId: revenueRequestId,
    ...
    // Create or update revenue_tracking records
    // First, check if revenue_tracking records exist for these sales
    const { data: existingTracking, error: trackingCheckError } = await supabase
      .from('revenue_tracking')
      .select('asset_buyer_id')
    ...
    if (missingTrackingIds.length > 0) {
      ...
      console.log('🔍 Creating missing tracking records:', {
        count: missingTrackingIds.length,
        records: trackingRecordsToInsert
      })
    ...
    console.error('Error submitting Stripe payment request:', error)
    toast({
      title: "Submission Failed",
...
const generations = useMemo(
  ...
  const createListing = async () => {
    ...
    // Double-check if asset is already listed by querying the database directly
    ...
    if (existingListing) {
      toast({ 
        ...
        description: "This card is already listed for sale.", 
        variant: "destructive" 
      })
      return
    }
    ...
    const { data, error } = await supabase
      ...
      .select("id, asset_id, seller_id, status, price_cents") 
      .returns<ListingRow[]>()
    setCreating(false)
    if (error) {
    ...
    <>
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
        ...
        {!uid ? (
          ...
          <Card className="bg-cyber-dark/60 border border-cyber-cyan/30">
            <CardContent className="p-6 text-gray-400">No uploads yet.</CardContent>
          </Card>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
{uploads.map((u, index) => {
    ...
    <Dialog open={sellOpen} onOpenChange={setSellOpen}>
      <DialogContent className="bg-cyber-dark/95 border-2 border-cyber-cyan/50 text-white">
        ...
        <div className="space-y-4">
          ...
          {/* Stripe Connect requirement removed - anyone can sell */}
        </div>
