...
// Lazy load the custom 3D preview wrapper component
...
const QUANTITY_STORAGE_KEY = 'cardify-custom-card-quantity'

// Create a flippable card preview component for custom uploaded images
function CustomCardPreview({ uploadedImage, cardFinish }: { uploadedImage: string | null, cardFinish?: 'matte' | 'rainbow' | 'gloss' }) {
  ...
  return (
    <div className="relative w-full mx-auto font-mono">
      ...
      >
        ...
        >
          {/* Front of Card - User's uploaded image */}
          <div className={
            isSmallScreen 
            ...
            <div className="relative w-full h-full rounded-xl border-2 border-cyber-cyan/50 shadow-2xl overflow-hidden">
              {/* User's uploaded image as the card front */}
              <img
                src={uploadedImage}
                alt="Custom Card"
...
export function CustomCardCheckoutModal({ isOpen, onClose, uploadedImage, processedImageBlob, uploadedImageUrl }: CustomCardCheckoutModalProps) {
  ...
  }, [isOpen, isVisible, uploadedImage, prevImage])
  ...
  const fetchInventoryData = useCallback(async (showLoading = false) => {
    ...
    try {
      ...
      if (!abortController.signal.aborted) {
        ...
        // No inventory limits for custom cards
      }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      setShowCase3DPreview(false)
      setFadeTransition({ isTransitioning: false, show3D: false })
      // Reset display case states
      setIncludeDisplayCase(false)
      setDisplayCaseQuantity(1)
    }
  }, [isOpen, inventoryState.error, fetchInventoryData])

  // Get current pricing information based on quantity
  const getCurrentPricing = () => {
    if (!inventoryState.data) {
      return {
        pricePerUnit: 9, // Fallback price for custom cards
        totalPrice: 9 * quantityState.value,
        discount: 0,
        savings: 0,
      }
    }
    ...
    const bestTier = applicableTiers.length > 0 
      ...
      : customCardData.pricingTiers[0] // Fallback to base price

    const pricePerUnit = bestTier.pricePerUnit
    
    // Add card finish pricing
    const cardFinishPricePerCard = (cardFinish === 'rainbow' || cardFinish === 'gloss') ? 4.00 : 0
    const totalPricePerUnit = pricePerUnit + cardFinishPricePerCard
    
    const cardsTotalPrice = totalPricePerUnit * quantityState.value
    const originalPrice = customCardData.pricePerUnit * quantityState.value
    const savings = originalPrice - (pricePerUnit * quantityState.value) // Savings only on base price, not style
    ...
    return {
      ...
      savings,
      cardFinishPrice: cardFinishPricePerCard,
    }
  }

  const currentPricing = getCurrentPricing()
  // Custom cards are made-to-order, no inventory limits
  const isSoldOut = false // Custom cards are never sold out
  ...
  const handleAddToCart = async () => {
    if (isSoldOut || !quantityState.isValid || !uploadedImage || !inventoryState.data) return

    const pricing = getCurrentPricing()

    // Use the pre-uploaded URL if available
    let finalImageUrl = uploadedImageUrl

    // If no pre-uploaded URL, upload now using unified function
    if (!finalImageUrl && uploadedImage) {
      try {
        ...
        const uploadResult = await unifiedImageUpload(imageBlob, {
          title: 'Custom Card',
          prompt: 'Custom card design'
        })
        finalImageUrl = uploadResult.publicUrl
      } catch (error) {
        console.error('Failed to upload image for cart:', error)
        // Fall back to using the original image
        finalImageUrl = uploadedImage
      }
    }

    // Use the uploaded URL or fall back to original
    const imageUrl = finalImageUrl || uploadedImage
  ...
  const handleCreateCheckout = async (address: ShippingAddress) => {
    if (isSoldOut || !quantityState.isValid || !uploadedImage) return
    ...
    console.log('🛒 Starting custom card checkout process:', logContext)
    
    try {
      // No inventory check needed for custom cards - they are made to order

      // Use the pre-uploaded URL if available
      let customImageUrl = uploadedImageUrl
      
      // If no pre-uploaded URL, upload now (fallback for backwards compatibility)
      if (!customImageUrl) {
        console.log('📤 No pre-uploaded URL found, uploading image now...')

        // Use the processed blob if available, otherwise fall back to fetching the preview image
        let imageBlob: Blob
        if (processedImageBlob) {
          console.log('Using pre-processed cropped image')
          imageBlob = processedImageBlob
        } else {
          console.log('Falling back to original image (no processed blob available)')
          imageBlob = await fetch(uploadedImage).then(r => r.blob())
        }

        // Use unified upload function that handles both guest and authenticated users
        try {
          const uploadResult = await unifiedImageUpload(imageBlob, {
            title: 'Custom Card',
            prompt: 'Custom card design'
          })
          customImageUrl = uploadResult.publicUrl
          console.log('✅ Upload successful:', customImageUrl)
        } catch (uploadError) {
      ...
      const response = await csrfFetch('/api/create-checkout-session', {
        ...
        body: JSON.stringify({ 
          ...
          customImageUrl: customImageUrl,
          cardFinish: cardFinish,
          returnUrl: window.location.pathname // Pass current page path
        }),
      })
            ...
            <CardContent className="p-0">
              ...
              <div className="text-center py-3 sm:py-4 px-4 border-b border-cyber-cyan/20">
                <div className="flex items-center justify-center gap-2 mb-2">
                  ...
                  <h1 
                    id="modal-title"
                    className="text-lg sm:text-2xl lg:text-3xl font-bold text-white tracking-wider glitch max-w-[200px] sm:max-w-none" 
                ...
                {currentStep === 'quantity' ? (
                  ...
                  <div className="flex justify-center order-1 lg:order-1">
                    <div className="relative w-full max-w-[200px] sm:max-w-xs lg:max-w-sm">
                      ...
                      >
                        <CustomCardPreview key={`2d-preview-${uploadedImage || 'placeholder'}`} uploadedImage={uploadedImage} cardFinish={cardFinish} />
                      </div>
                      ...
                      {showCase3DPreview && (
                        ...
                        >
                          <CustomCard3DPreviewWrapper
                            key={`3d-preview-${uploadedImage || uploadedImageUrl || 'placeholder'}-${animationKey}`}
                  ...
                  <div className="flex justify-center lg:items-center order-2 lg:order-2">
                    <div className="w-full max-w-sm space-y-3 sm:space-y-4 lg:space-y-5">
                      ...
                      <div className={`space-y-2 ${isSoldOut ? 'opacity-50' : ''}`}>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <Input
                            ...
                            aria-label={`Quantity (current: ${quantityState.value}, max: ${QUANTITY_CONFIG.MAX})`}
                      ...
                      <div className={`space-y-2 ${isSoldOut ? 'opacity-50' : ''}`}>
                        ...
                        <Select value={cardFinish} onValueChange={(value) => setCardFinish(value as 'matte' | 'rainbow' | 'gloss')} disabled={isSoldOut}>
                      ...
                      <div className={`space-y-2 ${isSoldOut ? 'opacity-50' : ''}`}>
                        ...
                        {currentPricing.savings > 0 && (
                          <div className="flex justify-between items-center text-sm text-cyber-purple">
                          ...
                          {inventoryState.data.customCard.pricingTiers.slice(1).map((tier, index) => (
                            <div key={index} className="flex justify-between">
                              ...
                              <span className="text-cyber-purple">
                                {tier.discount}% off (${tier.pricePerUnit.toFixed(2)}/card)
                              </span>
                            </div>
                          ))}
                        </div>
                      ...
                      {isSoldOut ? (
                        ...
                        <div className="flex flex-col lg:flex-row gap-3">
                          <Button
                            ...
                            disabled={isOrdering || !quantityState.isValid || !uploadedImage}
